...
{
	auto transform = Renderer::Camera::viewMatrix({ 0, 0, 0 }, { 0, 0, -1 }, RML::Vector::up());
	EXPECT_EQ(transform, RML::Transform().matrix());
}

TEST(CppRayTracerChallenge_Core_Renderer_Camera, view_matrix_looking_in_positive_z_direction)
{
	auto transform = Renderer::Camera::viewMatrix({ 0, 0, 0, }, { 0, 0, 1 }, RML::Vector::up());
	EXPECT_EQ(transform, RML::Transform().scale(-1, 1, -1).matrix());
}
...
TEST(CppRayTracerChallenge_Core_Renderer_Camera, view_matrix_transforms_the_world)
{
	...
	EXPECT_EQ(transform, RML::Transform().translate(0, 0, -8).matrix());
}
...
TEST(CppRayTracerChallenge_Core_Renderer_Camera, creating_a_camera)
{
	...
	EXPECT_EQ(camera.transformMatrix(), RML::Transform().matrix());
}
